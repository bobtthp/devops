...
import redis
pool = redis.ConnectionPool(host='192.168.122.3', port=6379, decode_responses=True)
...
def dojob(conn, address, thread_name):
    ...
    ssh = get_ssh('192.168.122.3','root','gzq123')   # 连接远程服务器
    ssh_t = ssh.get_transport()
    chan = ssh_t.open_session()
    chan.setblocking(0)   # 设置非阻塞
    log_command = 'tail -f /qingke/tomcats/%s_tomcat/logs/catalina.out' %  ('')
    print (log_command)
    chan.exec_command(log_command)
    while True:
...
def ws_service():
    ...
    while True:
        ...
        t.start()
        threading.Timer(299,send_data,args = [connection,'=======connection will be closed by timeout(timeout = 300s)=======']).start()
        threading.Timer(300, connection.close).start()
        index += 1
