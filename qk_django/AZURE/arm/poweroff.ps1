...
{
	#Write-Output ($vm.ResourceGroupName);
	if ($vm.PowerState -eq "VM running" -and $vm.name -ne "qks-138" -and $vm.name -ne "qks-159" -and $vm.name -ne "qks-160" -and $vm.name -ne "qks-mssql" -and $vm.name -ne "qks-hyapp" -and $vm.name -ne "qks-crmelive"  -and $vm.name -ne "qks-mobile"  -and $vm.name -ne "qks-ids"  -and $vm.name -ne "qks-150" -and $vm.name -ne "qks-openvpn"  -and $vm.name -ne "qks-rabbitredit" ){
		$VMs[$vm.ResourceGroupName]+=$vm.name+';'
	}
}
...
#重置所有状态
$sql = "update azurevm set state=0,delayedoff=0,closed=0,opened=0"
$command = New-Object MySql.Data.MySqlClient.MySqlCommand($sql, $connection)
$command.ExecuteNonQuery()
